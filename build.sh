...
rm -rf dist/hetangai dist/hetangai.app
uv run pyinstaller hetangai.spec --clean
if [ $? -ne 0 ]; then
    echo -e "${RED}打包失败${NC}"
    exit 1
fi
...
echo -e "\n${GREEN}=== 构建成功 ===${NC}"
echo -e "${GREEN}应用位置: ${YELLOW}dist/hetangai.app${NC}"
echo -e "${BLUE}提示: 可以将 hetangai.app 拖到应用程序文件夹${NC}"
